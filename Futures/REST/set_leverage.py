#!/usr/bin/env python3
"""
Bitget USDT Perpetual REST API - Set Leverage

Изменение плеча для торговой пары.
Плечо можно установить от 1x до 125x в зависимости от пары.

Документация: https://www.bitget.com/api-doc/contract/trade/Change-Leverage
"""
...
def set_leverage(symbol, leverage, margin_mode="crossed"):
    """
    Установка плеча для торговой пары
    
    Args:
        symbol (str): Торговая пара (например, BTCUSDT)
        leverage (int): Значение плеча (1-125)
    ...
    try:
        ...
        print(f"⚡ Установка плеча для {symbol}...")
        print(f"📊 Плечо: {leverage}x")
        print(f"💰 Режим маржи: {margin_mode}")
...
if __name__ == "__main__":
    ...
    result = set_leverage("BTCUSDT", 10, "crossed")
    if result:
        print("\\n✅ Тест прошел успешно!")
    else:
        print("\\n❌ Тест не прошел!")
